      ...
      <template #empty>
        <div class="empty-state">
          <p class="empty-state__desc">
            {{ total === 0 ? t('contacts.noAgenciesTitle') : t('contacts.noResultsFoundTitle') }}
          </p>
          <p class="empty-state__desc">
            {{
              total === 0
                ? t('contacts.noAgenciesMessage')
                : t('contacts.noResultsFoundMessage', {
                    entities: t('contacts.entities.agency', 2),
                  })
            }}
          </p>
  ...
  setup() {
    ...
    const fetchNow = async (): Promise<void> => {
      ...
      try {
        ...
        numTotalRecords.value = contactsStore.agenciesCount;
      } finally {
        if (id === currentRequest.value) {
