<template>
  <div class="page-invoices">
    ...
    <div class="page-container">
      ...
      <!-- <Dialog
        :show="isOpenPartnersMail"
        class="add-email-modal"
        @close="isOpenPartnersMail = false"
        :headerColor="'#FFFFFF'"
      >
        <template #header>
          <div class="add-email-title">Añadir destinatarios</div>
        </template>
        <template>
          <div class="mail-content">
            <div class="add-partner">
              <div class="search-partner-input">
                <Autocomplete
                  @textSearchChanges="getGuestFromVatDocNumber($event)"
                  @addNew="openPartnerDialog(0)"
                  @keypress.enter="addMailAddressToMails($event.target.value)"
                  id="partners-autocomplete-mail-invoice"
              ...
              <button
                ...
                v-if="showBtnToAddEmail && emailToAdd !== ''"
              >
                <CustomIcon
                  ...
                  height="16px"
                />
                <span class="add-partner-btn-text"> Añadir </span>
              </button>
            </div>
            <div class="partners-title">
              Destinatario{{ partners.length + emails.length > 1 ? 's' : '' }}:
            </div>
            <div class="partners" v-if="partners.length > 0 || emails.length > 0">
              <div class="partner" v-for="partner in partners" :key="partner.id">
                <div>
                  ...
                  <span class="add-email-address" v-else @click="openPartnerDialog(partner.id)">
                ...
                <span class="partner-name">
                  <div class="name-and-delete">
                    {{ partner?.name }}
                    <CustomIcon
                      imagePath="/app-images/icon-close-red.svg"
                      color="#FF0000"
                      width="16px"
                      height="16px"
            ...
            <div v-if="invalidAddress" class="error">
              <CustomIcon
                ...
                height="16px"
              />
              <span class="error-text"> Dirección de correo no válida </span>
            </div>
            <div v-if="partnerWithoutMailAddress" class="error">
              <CustomIcon
                imagePath="/app-images/icon-alert.svg"
                color="#FF0000"
                width="16px"
              ...
              <span class="error-text">
                El cliente no tiene una dirección de correo establecida
              </span>
            </div>
            <button
              class="send-email-btn"
              @click="sendOrPrintInvoicesByMail(true, false)"
              v-if="partners.length > 0 || emails.length > 0"
              :disabled="invalidAddress || partnerWithoutMailAddress"
            >
              Enviar factura{{ invoicesSelected.length > 1 ? 's' : '' }} por email
            </button>
          </div>
        </template>
      </Dialog> -->

      <!-- <Dialog v-model="showPartnerDialog" persistent class="q-dialog-partner">
        <PartnerDialog @partnerCreated="addMailToPartner()" />
      </Dialog> -->

      <!-- <Transition name="fade-mail-feedback-transition">
        <div class="mail-send-feedback-tooltip" v-if="showMailSendFeedback">
          <div class="mail-send-feedback-text">
            <span>Facturas enviadas por email correctamente</span>
          </div>
        </div>
      </Transition> -->
    </div>
...
import InputDateRange from '@/components/roomdooComponents/InputDateRange.vue';

import InvoiceDialog from '@/components/invoices/InvoiceChanges.vue';
import ListComponent from '@/components/roomdooComponents/ListComponent.vue';
...
import InvoiceMailComponent from '@/components/mails/IvoiceMailComponent.vue';
export default defineComponent({
  ...
  setup() {
    ...
    const openMailModal = () => {
      ...
      dialogService.open({
        header: 'Añadir destinatarios',
        content: markRaw(InvoiceMailComponent),
        props: {
          isOpenPartnersMail: true,
          partners: partners.value,
          emails: emails.value,
          invalidAddress: invalidAddress.value,
          partnerWithoutMailAddress: partnerWithoutMailAddress.value,
          showBtnToAddEmail: showBtnToAddEmail.value,
        },
        closable: true,
      });
    };
